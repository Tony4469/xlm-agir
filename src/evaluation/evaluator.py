...
class Evaluator(object):
    ...
    def evaluate_clm(self, scores, data_set, lang1, lang2):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2, stream=(lang2 is None)):
            ...
            print(8, x, lengths, positions, langs)
            # forward / loss
    ...
    def evaluate_mlm(self, scores, data_set, lang1, lang2):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2, stream=(lang2 is None)):
            ...
            # forward / loss
            print(7, x.size(), lengths, positions, langs)
            print(71, pred_mask.size())
            print(72, y.size())
            
            tensor = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False)
            word_scores, loss = model('predict', tensor=tensor, pred_mask=pred_mask, y=y, get_scores=True)
            print(8,tensor.size())
            print(9,word_scores.size(), loss.size() )
            # update stats
            n_words += len(y)
            xe_loss += loss.item() * len(y)
            n_valid += (word_scores.max(1)[1] == y).sum().item()
...
def souf_evaluate_mlm(self, scores, data_set, lang1, lang2):
        """
        Evaluate perplexity and next word prediction accuracy.
        """
        params = self.params
        ...
        for batch in self.get_iterator(data_set, lang1, lang2, stream=(lang2 is None)):
            ...
            # forward / loss
            print(7, x, lengths, positions, langs)
            tensor = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False)
            word_scores, loss = model('predict', tensor=tensor, pred_mask=pred_mask, y=y, get_scores=True)
            print(8,tensor)
            print(9,word_scores, loss )
            # update stats
            n_words += len(y)
            xe_loss += loss.item() * len(y)
            n_valid += (word_scores.max(1)[1] == y).sum().item()
