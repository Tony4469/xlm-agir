...
class PredLayer(nn.Module):
    ...
    def __init__(self, params):
        super().__init__()
        print('params.asm', params.asm)
        print('params.n_words', params.n_words)
        print('params.pad_index', params.pad_index)
        print('params.emb_dim', params.emb_dim)
        self.asm = params.asm
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        """
        super().__init__()
        self.training = False #A changer Souf
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None):
        ...
        tensor = self.layer_norm_emb(tensor) #A voir
        tensor = F.dropout(tensor, p=self.dropout, training=self.training)  #A voir : training = self.training
        tensor *= mask.unsqueeze(-1).to(tensor.dtype)

        # transformer layers
        #self.n_layers=1 #A supprimer
        print('self.attention_dropout', self.attention_dropout)
        print('self.training', self.training)
        print('self.n_layers', self.n_layers)
        for i in range(self.n_layers):
            # self attention
            attn = self.attentions[i](tensor, attn_mask, cache=cache) #A réactiver si beosin, souf
